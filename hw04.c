...
long long int distance (const DataPoint * datapoint, const Centroid * centroid)
{
  ...
  long long int partSum = 0; 		// intialize partSum to zero 
  int dim = datapoint -> dimension; 
  //Figure out Sum by usng distance formula 
  for(int i = 0; i < dim; i++) 
  {
    partSum = pow((datapoint -> data[i] - centroid -> data[i]), 2); 
    sum += partSum; 
  }
...
int closestCentroid (int kval, DataPoint * datapoint, Centroid * *centroids)
{
  ...
  long long int minDistance = 0; //miniumum distance   
  long long int distancePoint = 0; 
  ...
  {
    distancePoint = distance(datapoint, centroids[i]); 
   printf("minDistance %lld\n", minDistance);
  printf("distancePoint %lld\n", distancePoint); 
...
void kmean (int kval, int nval, DataPoint * *datapoints, Centroid * *centroids)
{
		// reset all centroids
	        long long int x =0;
   		x = distance(*datapoints, *centroids); 
		printf("%lld\n", x);
         
 		int dumb = closestCentroid(kval, datapoints[2], centroids); 
		printf("%d\n", dumb); 
